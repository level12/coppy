...
- mise
    - Uses `python.uv_venv_auto = "create|source"` to create and activate uv's project
      environment.
    - Gets its Python tool spec from `.python-version`.
    - Uses an `enter` hook to run `uv sync`
- Add a `python_version_min` Copier template setting so the `pyproject.toml` Python spec
  can be different from the interpreter version used for local development.
